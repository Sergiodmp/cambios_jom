...
def transport_cost(to, From_location):
    ...
    costes_df.reset_index(drop=True,inplace=True)
    return costes_df
...
def set_process_df(df, ca_counter_bid_df, ca_to_tr_df):
    ...
    new_line_df = pd.Series([np.nan, np.nan, np.nan, np.nan, np.nan, \
                    ...
                    index=['fab_start','processing_time','start_auction_before', \
                           ...
                           'T1', 'T2', 'T3', 'T4', 'T5', 'q'])
    process_df = pd.concat([df,new_line_df],axis=0, ignore_index=True)
    process_df.reset_index(drop=True,inplace=True)
    uind = process_df.index[-1]
    pind = process_df.index[-2]
    lrg  = 0.
    if 'largo' in ca_counter_bif_df.columns:
        lrg  = ca_counter_bid_df.loc[0, 'largo']
    if 'coil_length' in ca_counter_bif_df.columns:
        lrg  = ca_counter_bid_df.loc[0, 'coil_length']
    #
    processing_time = (1/ca_counter_bid_df.loc[0, 'setup_speed']) * lrg
    process_df.loc[uind,'start_auction_before'] = 6.5 * 60
    start_auction_before = process_df.loc[uind,'start_auction_before']
    process_df.loc[uind,'processing_time'] = processing_time
    if process_df.shape[0] < 2:
        process_df.loc[uind,'fab_start'] = datetime.datetime.now() + \
                          datetime.timedelta(minutes=1.)
    else:
        process_df.loc[uind,'fab_start'] = process_df.loc[pind,'fab_end']

    process_df.loc[uind,'fab_end']= process_df.loc[uind,'fab_start'] + \
                          datetime.timedelta(seconds=processing_time)
    if process_df.shape[0] < 2:
        process_df.loc[uind,'start_auction_before'] = datetime.datetime.now()
    else:
        process_df.loc[uind,'start_auction_before'] = process_df.loc[pind,\
                          'start_auction_before']
    start_next_auction_at = process_df.loc[uind,'fab_end'] - \
                          datetime.timedelta(seconds=start_auction_before)
    process_df.loc[uind,'start_next_auction_at'] = start_next_auction_at
    a = process_df.loc[uind,'fab_start']
    process_df.loc[uind,'setup_speed']= ca_counter_bid_df.loc[0, 'setup_speed']
    return process_df
...
def append_bid(bid, bid_register_df, agent_name, agent_full_name, ca_agent_df, bid_level, *args):
    ...
    if bid_level == 'initial':
        ...
        bid_register_df.reset_index(drop=True, inplace=True)
